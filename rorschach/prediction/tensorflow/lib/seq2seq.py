  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        return outputs + state_list, encoder_state
...
def attention_decoder(decoder_inputs,
  ...
  return outputs, state, attention_output
  ...
  with variable_scope.variable_scope(
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        if nest.is_sequence(state):
          state_list = nest.flatten(state)
        return outputs + state_list, attention_output

    outputs_and_state, attention_output = control_flow_ops.cond(feed_previous,
                                              lambda: decoder(True),
                                              lambda: decoder(False))
    outputs_len = len(decoder_inputs)  # Outputs length same as decoder inputs.
    state_list = outputs_and_state[outputs_len:]
    ...
    if nest.is_sequence(encoder_state):
      state = nest.pack_sequence_as(
          structure=encoder_state, flat_sequence=state_list)
    return outputs_and_state[:outputs_len], state, attention_output
